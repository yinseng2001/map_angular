...
App.controller("MapController", function ($scope, $location, $localStorage, $geolocation, uiGmapIsReady) {
    ...
    $scope.getCenter = function () {
        ...
        if(map.getCenter().lat() && map.getCenter().lng()){
            $localStorage.savedPos = {
                ...
                'reallat' : $scope.myPosition.coords.latitude,
                'reallng' : $scope.myPosition.coords.longitude
            }
    ...
    $scope.gotoPos = function () {
        ...
        if($scope.marker.coords.latitude && $scope.marker.coords.longitude){
            var point = new google.maps.LatLng($scope.marker.coords.latitude, $scope.marker.coords.longitude);
            if(map){
                map.panTo(point);
            }
            ...
            var point = new google.maps.LatLng($scope.map.center.latitude, $scope.map.center.longitude);
            if(map){
                map.panTo(point);
            }
        }
